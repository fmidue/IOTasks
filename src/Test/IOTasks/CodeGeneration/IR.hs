...
data IRTag
  = READ IndexedVar
  | UPDATE IndexedVar (Varname -> Varname -> AST) IndexedVar IndexedVar -- UPDATE x f y v assigns the result f applied to y and v to x, ie. x := f y v
  | PRINT AST
  | IF AST IR IR
...
irRead x = IR [READ x]
irUpdate :: IndexedVar -> (Varname -> Varname -> AST) -> IndexedVar -> IndexedVar -> IR
